...
- No HTTPS/TLS termination in this local compose setup — that's a reverse-proxy/load-balancer concern for the actual cloud environment (e.g. an AWS ALB), not something to add locally.
...
5. Terminate TLS at an Application Load Balancer in front of the frontend (and/or backend) service; update `ALLOWED_HOSTS`/`CORS_ALLOWED_ORIGINS`/`CSRF_TRUSTED_ORIGINS` to the real domain.
